...
# Sample Summaries
(samp_summ_fixes <- evt0 %>% 
    mutate(ts = ymd_hms(timestamp),
           wk = week(ts)) %>% 
    group_by(individual_id, wk) %>% 
    summarize(n = n(),
              species = species[1]) %>% 
    ungroup() %>% 
    group_by(species) %>% 
    summarise(avg_fixes = mean(n),
              min_fixes = min(n),
              max_fixes = max(n)))
...
for(j in 1:length(species_list)){
  ...
  evt_wk <- evt_sp %>% 
    ...
    summarize(dist2water_scale = mean(dist2water_scale, na.rm = T), 
              ...
              yr = yr[1]) %>% 
    group_by(wk) %>% # median individuals 
    summarize(
...
# Save out PCAs
save(pca_out, file = "out/fitted_pcas.rdata")
...
ggsave(loading_plot_fig, filename = "out/loading_plot_fig-poly.png", width = 8, height = 6, units = "in")

(track_plot_fig <- track_plots[[1]] + track_plots[[3]] + track_plots[[4]] + track_plots[[5]] + 
    plot_layout(ncol = 1, nrow = 4, guides = "auto", widths = c(4,4), heights = c(3,3)) & theme(axis.title = element_blank())) 

ggsave(track_plot_fig, filename = "out/track_plot_fig-poly.png", width = 8, height = 6, units = "in")
...
ggsave(plot = fig3, filename = "out/fig3.png", width = 6, height = 3, units = "in")
ggsave(plot = fig3, filename = "out/fig3.pdf", width = 8, height = 4, units = "in")
...
ggsave(scores_fig, file = "out/fig4 - scores-fig.png", width = 8, height = 7, units = "in")
ggsave(scores_fig, file = "out/fig4 - scores-fig.pdf", width = 8, height = 7, units = "in")
